...
- 🎯 **应用预设** — 钠/锂离子电池、固态电解质、硫化物/卤化物/NASICON/石榴石候选、光伏光催化半导体等
- 🏷️ **结构分类** — 晶系（7 类）、维度（3D/2D/1D/0D/插层）多选
- 📈 **全量概览** — 顶栏带隙分布条形图、晶系/维度统计
- 🧬 **3D 晶体结构预览** — 从本地 `by_id.zip` 读取 CIF，Three.js 球棍模型 + 晶胞框交互查看
- 🔋 **固态电池候选识别** — 按 Li/Na 硫化物、卤化物、NASICON、石榴石等候选族筛选并给出推荐理由
- 💾 **导出** — 查询结果或单条材料导出为 CSV / JSON，结构详情支持导出 CIF / VASP POSCAR / Quantum ESPRESSO 输入模板
...
GNoME 原始数据为 `stable_materials_summary.csv`（约 151MB），请从 GNoME 官方发布获取，放入 `data/gnome/` 目录。若要启用 3D 结构预览，还需要下载结构包 `by_id.zip` 并放入同一目录。
...
| 固态电解质候选 | Li/Na 硫化物、卤化物、NASICON、石榴石族 · 非金属 · 分解能 ≤ 0.1 eV | 固态电池电解质候选缩小 |

> `分解能 ≤ 0.1 eV/atom` 是「距凸包足够近、可能可合成」的经验阈值。

## 已知限制

- **3D 结构依赖本地结构包**：预览功能从 `data/gnome/by_id.zip` 或环境变量 `GNOME_ZIP` 读取 CIF；缺少该文件时仍可正常筛选材料。
- **结构预览为快速预览**：当前显示 CIF 原始原子坐标和经验距离键连线，未做空间群对称性展开或严格化学键判定。
...
- [x] 3D 晶体结构本地渲染（Three.js + by_id.zip CIF）
- [x] 导出 VASP POSCAR / Quantum ESPRESSO 输入文件模板
